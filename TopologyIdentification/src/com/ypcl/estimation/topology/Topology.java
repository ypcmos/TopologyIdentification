...
public class Topology implements ITextDataSource, Serializable {
	...
	transient protected List<Record> mRecords;
	protected List<Record> badDataRecords = new LinkedList<Record>();
	protected double s;
	transient protected Matrix invG, R;
	...
	public static double e = 1.0e-5, delta = 1e-4;
	...
	public Topology initialY() {
		...
		for (Entry<Integer, Bus> entry : getBuses().entrySet()) {
			...
			for (int link : links) {
				...
				Complex ympart = y.mul(-1);
				
				Complex ground = null;
				
				if (left) {
					ground = new Complex(branch.getGl(), branch.getBl());
				} else {
					ground = new Complex(branch.getGr(), branch.getBr());
				}
				yr.set(i - 1, link - 1, y.getReal());
				yi.set(i - 1, link - 1, y.getImag());
				ym = ym.add(ground).add(ympart);
			}
