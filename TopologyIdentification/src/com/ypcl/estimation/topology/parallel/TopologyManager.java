...
public class TopologyManager extends Topology {
		...
		while (null != (line = br.readLine())) {
			...
			if (!line.isEmpty()) {
				...
				if (line.contains("-")) {
					...
					g_l = branch.getGl();
					g_r = branch.getGr();
					Branch b1 = new Branch(i, index, r / 2, x / 2, 0),
							b2 = new Branch(j, index, r / 2, x / 2, 0);
					b1.setBl(b_l);
					b1.setGl(g_l);
	...
	public static double citaij(TopologyUnit t1, TopologyUnit t2, int pvi) {	
		...
		double gij = y.getReal(), bij = y.getImag(),  gi0 = b1.getGl(), gj0 = b2.getGl();
	...
	static public TopologyUnit mergeBlocks2(TopologyUnit t1, TopologyUnit t2) {		
		...
		while (it.hasNext()) {
			...
			int id = bus.getId();
			if (bus.getType() == Bus.VIRTUAL) {
				//System.out.println(bus.getId() + "v:" + bus.getData(Bus.V_CAL) + " cita:" + bus.getData(Bus.CITA_CAL));
				Bus b = t1.getBuses().get(id);
				
				if (b != null) {
					//System.out.println(b.getId()  + "-v:" + b.getData(Bus.V_CAL) + " cita:" + b.getData(Bus.CITA_CAL));
					it.remove();
					...
					double sub = b2.getData(Bus.CITA_CAL) - b1.getData(Bus.CITA_CAL)
							+ citaij(t1, t2, id);
					//System.out.println("*****" + citaij(t1, t2, id));
					sum += sub;
					s2.add(sub);
					t1.removeBranch(id);
					t2.removeBranch(id);
	...
	public boolean execute(final int times) throws IOException, InterruptedException {
		...
		for (Branch branch : borders) {
			...
			buses.get(pair.getI()).putData(Bus.P_CAL, calPi(i)).putData(Bus.Q_CAL, calQi(i));
			buses.get(pair.getJ()).putData(Bus.P_CAL, calPi(j)).putData(Bus.Q_CAL, calQi(j));
		}
		return true;
	}
	...
	public static void main(String [] args) throws Exception {
		...
		t.makeBlocksConfigure("E:/学习资料/研究生大本营/毕业论文/code_data/distributed_conf_14_3.txt", conf);
		t.prepareBlock(conf);
		//System.out.println(t.kaTable);
		if (t.execute(1000)) {
			//t.correctPhaseAngle(30 / 180.0 * Math.PI);
			t.toFile("E:/学习资料/研究生大本营/毕业论文/code_data/ieee_t_output_" + t.name + ".csv");
		} else {
