...
public class Graph<T>  implements Serializable {
	...
	public static class GraphNode<T> implements Serializable {
		...
		public GraphNode<T> delConnection(T other) {
			set.remove(other);
			return this;
		}
	...
	public Graph(Graph<T> g) {
		for (Entry<T, GraphNode<T>> h : g.heads.entrySet()) {
			heads.put(h.getKey(), new GraphNode<T>(h.getValue()));
		}
	}
	...
	public Graph<T> delConnection(T first, T second) {
		GraphNode<T> node1 = heads.get(first), node2 = heads.get(second);
		
		if (node1 == null || node2 == null) {
			throw new NodeNotInGraphException();
		}
		node1.delConnection(second);
		node2.delConnection(first);
		return this;
	}
	...
	public Graph<T> separateChildGraph(Graph<T> childGraph) {
	...
	public List<Graph<T>> pieces(List<Pair<T>> pairs) {
		List<Graph<T>> list = new LinkedList<Graph<T>>();
		//System.out.println(this);
		for (Pair<T> pair : pairs) {
			T i = pair.getI(), j = pair.getJ();
			delConnection(i, j);
		}
		...
		while (mother.size() != 0) {
			List<T> ns = mother.bfs();
			//System.out.println(ns);
			Graph<T> g = mother.childGraph(ns);
			//System.out.println(g);
			list.add(g);
			mother = mother.separateChildGraph(g);
		}
		return list;
	}
	...
	public List<Graph<T>> pieces(int n) {
		...
		for (int i = 0; i < n - 1; i++) {
			...
			list.add(target);
			mother = mother.separateChildGraph(target);	
