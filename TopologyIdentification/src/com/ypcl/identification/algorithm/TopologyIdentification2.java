...
public class TopologyIdentification2 implements Serializable {
	...
	protected double basicDelta = 1.96, d;
	protected TopologyIdentification2() {
		
	}
	
	public TopologyIdentification2(String path) throws IOException {
	...
	public TopologyIdentification2(ITopologySimulator simulator) {
		pDirtyBuses = new TreeMap<Integer, Double>();
		pDirtyBranches = new HashMap<IntPair, Double>();
	...
	public Graph<Integer> getTopology() {
		return topology;
	}
	...
	protected double error(int size) {
		return Math.sqrt(d * size) * basicDelta;
	}
	public boolean validateBusUsingSimpleRule(int id, List<IntPair> pairs) {
		return distanceBetweenNodeAndBranchesSuper(id, pairs) < error(1 + pairs.size()) + 5;
	}
	...
	public boolean validateBranchUsingSimpleRule(IntPair pair) {
		...
		return /*numberCheck && */distanceBetweenBranches(pair) < error(2) + 0.05 * Math.min(Math.abs(getBranchValue(pair).value), Math.abs(getBranchValue(pair.exchange()).value));
	}
	...
	public boolean checkSubBetweenBuses() {
		...
		for (Entry<IntPair, Double> braEntry : forLoop.entrySet()) {
			...
			if (Math.abs(pis + pjs) <= error(topology.getNodeLinks(pair.getI()).size() + topology.getNodeLinks(pair.getJ()).size())) {
	...
	private void fromSimulator(ITopologySimulator simulator) {
		Map<Integer, Double> pBuses = simulator.getBusPowersMap();
		Map<IntPair, Double> pBranches = simulator.getBranchPowersMap();
	...
	public static void main(String[] args) throws IOException {
		TopologyIdentification2 ti = new TopologyIdentification2("D:/graduate/node_branch_P.txt");
		Graph<Integer> g = ti.topology.childGraphKeepConnection(8);
		System.out.println(g);
		System.out.println(ti.topology.separateChildGraph(g));
